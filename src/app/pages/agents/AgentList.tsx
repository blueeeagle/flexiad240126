...
//   const BankVerification = queryParams.get('bankVerified') || '';
//   console.log(BankVerification);

//   const getData =useCallback( async () => {
//     setLoading(true); // Show loader
//     let query = `?pageIndex=${page}&pageSize=${pageSize}&searchValue=${searchValue}`;

//     if (BankVerification === 'false') {
//       query += `&bankVerified=false`;
//     }
//     try {
//       const countryData = await getRequest(`/agent/list`, query);
...
//       setLoading(false); // Hide loader in case of error
//     }
//   },[BankVerification, page, searchValue])
...
//       renderCell: (params: any) => (
//         <Link
//           to={`/agent/${params.row._id}?isverified=${params.row.isVerified ?? "false"
//             }&company=${params.row.companyName.replace(/\s+/g, "-").toLowerCase() || "n-a"
//             }`}
//         >
...
import { Link, useLocation, useNavigate } from "react-router-dom";
import { stringToDate } from "../../../common/Date";
import { getRequest, patchRequest, deleteRequest } from "../../modules/auth/core/_requests";
import ReactPaginate from "react-paginate";
import { AiFillLeftCircle, AiFillRightCircle } from "react-icons/ai";
import { IconContext } from "react-icons";
import logo from "../../../../src/_metronic/assets/sass/components/logoimage/logo.png";
...
const AgentList: FC = () => {
  ...
  console.log("AgentList Permissions:", { canView, canEdit, canDelete, canCreate });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (!canEdit) return;
      setLoading(true);
      let reqBody = { reason: values?.reason };
      try {
        if (companyId) {
          const response = await patchRequest(`/agent/company/reject/${companyId}`, reqBody);
          if (response?.data?.status === "ok") {
            setIsSuccess(true);
            setSuccessMsg("Status Updated successfully");
            await getData();
  ...
  const agentApprove = async (id: any) => {
    if (!canEdit) return;
    try {
      const response = await patchRequest(`/agent/company/approve/${id}`, "");
      if (response?.data?.status === "ok") {
        setIsSuccess(true);
        setSuccessMsg("Status Updated successfully");
        await getData();
      ...
      setIsFailed(true);
      setErrorMsg(`Error approving agent`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteAgent = async (id: string) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        setLoading(true);
        try {
          const response = await deleteRequest(`/agent/company/${id}`);
          if (response?.data?.status === "ok") {
            setIsSuccess(true);
          ...
          setIsFailed(true);
          setErrorMsg("Error deleting agent");
        } finally {
          setLoading(false);
        }
      }
  ...
  const baseColumns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 200,
      renderCell: (params: any) => {
        const linkTarget = `/agent/${params.row._id}?isverified=${params.row.isVerified ?? "false"
          }&company=${params.row.companyName.replace(/\s+/g, "-").toLowerCase() || "n-a"
          }`;
        if (canEdit) {
          return <Link to={linkTarget}>{params.row.companyName || "N/A"}</Link>;
        }
        return <span>{params.row.companyName || "N/A"}</span>;
      },
    },
    {
      field: "services",
  ...
  // Add Actions column only if user can edit, delete, or approve/reject
      ...
      renderCell: (params: any) => (
        <div className="action-dropdown">
          <select
            ...
            onChange={(e) => {
              ...
              if (selectedValue === "edit" && canEdit) {
                ...
                deleteAgent(params.row._id);
              } else if (selectedValue === "accept" && canEdit) {
                handleApprove(params.row._id);
              } else if (selectedValue === "reject" && canEdit) {
                handleRejectModal(params.row._id);
              }
          ...
          >
            ...
            {canEdit && params.row.status !== "Approved" && (
              <>
                <option value="accept">Accept</option>
                <option value="reject">Reject</option>
              </>
  ...
  return (
    <>
      ...
      <div className="row g-5 g-xl-8">
                ...
                <div className="">
                  <DataGrid
                    ...
                    sx={{
                      ...
                      "& .MuiDataGrid-columnHeader:focus-visible, & .MuiDataGrid-cell:focus-visible":
                        { outline: "none", border: "none", backgroundColor: "transparent" },
                      "& .MuiDataGrid-cell:active": { outline: "none", border: "none" },
                    }}
                  />
                </div>
              )}
        ...
        <Modal isOpen={isOpen} toggle={toggle}>
          <form onSubmit={formik.handleSubmit} noValidate className="form">
            <h2 style={{ display: "flex", justifyContent: "center" }}>Rejected</h2>
            <h4 style={{ display: "flex", justifyContent: "center" }} className="p-5 pt-10">
              Are you sure you want to reject the agent?
            </h4>
            <div className="row mb-12">
              <label className="col-lg-4 col-form-label required fw-bold fs-6">Reason</label>
              <div className="col-lg-12">
                <div className="row">
                  <div className="col-lg-12 fv-row">
                    <textarea
