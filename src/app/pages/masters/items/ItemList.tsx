...
//     const imageUrl = params.row.productImageURL
//       ? `http://adminapi.flexiclean.me/${params.row.productImageURL}`
//       : logo;
...
const ItemList: FC = () => {
  ...
  const currencyPerms = permissions.subMenu["Items"] || [];
  ...
  console.log("ItemList Permissions:", { canView, canEdit, canDelete, canCreate });
  ...
  const deleteItem = async (ID: string) => {
    ...
    if (window.confirm("Are you sure to delete this record?")) {
      try {
        const response = await deleteRequest(`/master/product/${ID}`);
        if (response?.data?.status === "ok") {
          setIsSuccess(true);
          setSuccessMsg("Item has been deleted successfully");
          await getData();
