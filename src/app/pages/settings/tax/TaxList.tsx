...
const TaxList: FC = () => {
  ...
  console.log("TaxList Permissions:", { canView, canEdit, canDelete, canCreate });
  ...
  const deleteTaxItem = async (ID: string) => {
    ...
    if (window.confirm("Are you sure to delete this record?")) {
      setIsLoading(true);
      try {
        const response = await deleteRequest(`/master/tax/${ID}`);
        if (response?.data?.status === "ok") {
          setIsSuccess(true);
          setSuccessMsg(`Tax item has been deleted successfully`);
          await getData();
        ...
        setIsFailed(true);
        setErrorMsg(`Error deleting tax item`);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const baseColumns: GridColDef[] = [
    {
      ...
      renderCell: (params: any) => {
        ...
        return <span>{params.row?.taxName}</span>;
      },
    },
    {
      field: "serviceName",
