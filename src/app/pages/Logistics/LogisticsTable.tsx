...
const LogisticsTable: React.FC<LogisticsTableProps> = ({ data }) => {
  ...
  console.log("LogisticsTable Permissions:", { canView, canEdit, canDelete, canCreate });

  // Block table if user cannot view
  if (!canView) {
    return (
  ...
  const handleOrderClick = (orderNo: string) => {
    // Only navigate if user has view permission (already checked above)
    navigate(`/customer/order/${orderNo}`);
  };
            ...
            <div>
              {selectedOrder.map((ord, idx) => (
                ...
                >
                  <strong>Status:</strong> {ord.status} <br />
                  <strong>Updated At:</strong>{" "}
                  {ord.updated_at ? new Date(ord.updated_at).toLocaleString() : "-"}
                </div>
              ))}
            </div>
          ) : (
