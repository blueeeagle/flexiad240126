...
//     const url = `https://adminapi.flexiclean.me/api/v1/activities/giftCard/${id}`;
...
const GiftCards: FC = () => {
  ...
  const getData = useCallback(async () => {
    ...
    try {
      ...
      setIsFailed(true);
      setErrorMsg("Error fetching gift cards");
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  const handleChangeStatus = async (id: string, isActive: boolean) => {
    if (!canEdit) return;
    const url = `https://adminapi.flexiclean.me/api/v1/activities/giftCard/${id}`;
    const updatedStatus = isActive ? "true" : "false";
    if (!token) {
      console.error("No token found");
      Swal.fire("Error", "No token found, please log in", "error");
      return;
  ...
  const baseColumns = [
    ...
    },
    {
      ...
      headerName: "Created On",
      width: 200,
      renderCell: (params: any) => params.row.updated_at || "",
    },
    {
      field: "isActive",
      headerName: "Status",
      ...
      renderCell: (params: any) => (
        <Switch
          ...
          onChange={() => {
            const newIsActive = !params.row.isActive;
            handleChangeStatus(params.row.id, newIsActive);
          }}
          disabled={!canEdit}
          inputProps={{ "aria-label": "controlled" }}
        />
            ...
            onChange={async (e) => {
              ...
              if (selectedOption === "edit" && canEdit) {
                const rowData = {
                  ...
                  currencyId: params.row.currencyId,
                };
                localStorage.setItem("selectedGiftCard", JSON.stringify(rowData));
                navigate(`/activities/giftCard/${giftCardId}`);
