...
const AgentPayOut: FC = () => {
  ...
  console.log("AgentPayOut Permissions:", { canView, canEdit, canDelete, canCreate });
  ...
  const handleUpdatePayout = async () => {
    if (!canEdit) return; // Prevent update if user lacks edit permission
    ...
    if (selectedTransaction) {
      ...
      if (selectedTransaction.status === "approved") {
        transactionData = {
          status: "approved",
          transactionId: selectedTransaction.transactionId || "JD993090489300930",
          amount: selectedTransaction.amount,
          date: selectedTransaction.date,
          comments: selectedTransaction.comments || "",
        };
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 180,
      renderCell: (params) => {
        const currencySymbol = params?.row?.currencyId?.currencySymbol || '-';
        const decimalPoints = params?.row?.currencyId?.decimalPoints || 0;
        return `${Number(params.value).toFixed(decimalPoints)} ${currencySymbol}`;
      }
    },
    {
      field: "comments",
      ...
      width: 200,
      renderCell: (params) => (
        <div>{params.row.comments || params.row.remarks || "-"}</div>
      ),
    },
    {
      field: "is_active",
      ...
      renderCell: (params) => (
        <Switch
          checked={params.row.is_active}
          disabled // Optional: disable if you want, could be controlled by canEdit
          inputProps={{ "aria-label": "controlled" }}
        />
      ),
    },
            ...
            <Form>
              ...
              <Form.Group controlId="formStatus">
                ...
                <Form.Control
                  ...
                  value={selectedTransaction.status}
                  onChange={(e) => handleStatusChange(e.target.value)}
                  disabled={selectedTransaction.status !== "pending" || !canEdit}
                >
                  <option value="pending">Pending</option>
                  <option value="approved">Approved</option>
                  <option value="rejected">Rejected</option>
