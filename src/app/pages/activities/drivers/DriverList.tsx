...
//     setLoading(true);
//     const driverListData = await getRequestPost(`/activities/drivers?pageIndex=${page}&pageSize=${pageSize}`, ``);
//     setLoading(false);


//     setTotal(driverListData?.data?.totalCount)
//     if (driverListData?.data?.status === "ok") {
//       setRowData(driverListData?.data?.data);
//     }
//   }, [page])
...
//     const url = `https://adminapi.flexiclean.me/api/v1/activities/driver/672129d2f8759d6d24574ff1`;
//     const payload = {
//       data: {
//         name: "flexi",
...
const ActivitiesDriverList: FC = () => {
  ...
  console.log("Driver Permissions:", { canView, canEdit, canDelete, canCreate });
  ...
  const deleteDriver = async (ID: string) => {
    ...
    if (window.confirm("Are you sure to delete this record?")) {
      ...
      try {
        ...
        setIsFailed(true);
        setErrorMsg("Error deleting driver");
      } finally {
        setLoading(false);
      }
    }
  ...
  const getData = useCallback(async () => {
    setLoading(true);
    try {
      const driverListData = await getRequestPost(
        `/activities/drivers?pageIndex=${page}&pageSize=${pageSize}`,
        ""
      );
      setTotal(driverListData?.data?.totalCount || 0);
      if (driverListData?.data?.status === "ok") {
        setRowData(driverListData?.data?.data);
      }
    } catch (error) {
  ...
  const handleChangeStatus = async (id: string, newStatus: boolean) => {
    ...
    // Fixed URL from original code (should be dynamic in production)
    const url = `https://adminapi.flexiclean.me/api/v1/activities/driver/672129d2f8759d6d24574ff1`;
    const payload = {
      data: {
        name: "flexi",
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok && result?.status === "ok") {
        Swal.fire("Success", "Driver status updated successfully!", "success");
        setRowData((prevData) =>
          prevData.map((item) =>
            item._id === id ? { ...item, is_active: newStatus } : item
          )
  ...
  const baseColumns: GridColDef[] = [
    ...
    },
    {
      ...
      headerName: "Created At",
      width: 150,
      renderCell: (params) => format(new Date(params.row.updated_at), "dd/MM/yyyy"),
    },
    {
      field: "is_active",
      headerName: "Status",
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (selectedOption === "edit" && canEdit) {
                localStorage.setItem("editDriverData", JSON.stringify(params.row));
                navigate(`/activities/driver/${params.row._id}`, {
                  state: params.row,
                });
